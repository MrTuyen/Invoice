...
{
    public class TempInvoiceController : BaseController
    {
        // GET: TempInvoice
        ...
        public ActionResult Downloadfile(string link, string pageSize)
        {
            try
            {
                string formCode = "01GTKT0/001";
                ...
                if (objUser.USINGINVOICETYPE == (int)EnumHelper.AccountObjectType.HOADONTRUONGHOC || objUser.USINGINVOICETYPE == (int)EnumHelper.AccountObjectType.HOADONTIENNUOC)
                {
                    formCode = "02GTTT0/001";
                    symbolCode = "AA/";
                    pageSize = "A5";
                ...
                var content = RenderViewToString(ControllerContext, link, model, true);
        ...
        public IDictionary<string, string> Parse(string css)
        {
            int pos = 0;
            while (css.IndexOf('{', pos) > -1)
            {
                int posChar = css.IndexOf('{', 0);
        ...
        private IEnumerable<Match> GetSelectorMatches(string css)
        {
            return Regex.Matches(css, @"(?<selectors>[a-z0-9_\-\.,\s#]+)\s*{(?<declarations>.+?)}",
                RegexOptions.IgnoreCase).Cast<Match>();
        }

        private IEnumerable<string> ExtractSelectorsByPattern(Match match, string pattern)
        {
            return Regex.Split(match.Groups["selectors"].Value, pattern).Where(s => !string.IsNullOrEmpty(s));
        }

        private IEnumerable<KeyValuePair<string, string>> SelectorToDeclarationValue(
