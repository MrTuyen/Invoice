...
namespace WebSockets.Common
{
    public abstract class WebSocketBase
    {
        ...
        {
            _logger.Information(this.GetType(), "1");
            _socket = socket;
            _stream = stream;
        ...
        private static StringBuilder dataFromChrome;
        private void MainReadLoop()
        {
            ...
            // why true de nhan du lieu tu chrome do chrome gui tung doan du lieu 1
            while (true)
            {
                ...
                if (frame.OpCode == WebSocketOpCode.ContinuationFrame)
                {
                    switch (_multiFrameOpcode)
                    {
                        case WebSocketOpCode.TextFrame:
                            ...
                            dataFromChrome.Append(data);
                            //OnTextMultiFrame(data, frame.IsFinBitSet);
                            if (frame.IsFinBitSet)
                            {
                                OnTextFrame(dataFromChrome.ToString());
                                dataFromChrome = null;
                            }
                            break;
                    ...
                    switch (frame.OpCode)
                    {
                        ...
                        case WebSocketOpCode.TextFrame:
                            ...
                            else
                            {
                                ...
                                //OnTextMultiFrame(data, frame.IsFinBitSet);
                                dataFromChrome = new StringBuilder(data);
                            }
                            break;
                        case WebSocketOpCode.BinaryFrame:
                            if (frame.IsFinBitSet)
